...
def get_args(configFileName):
    p = ArgumentParser(
        description='Import an mbox file into GroupServer.',
        epilog='Usually %(prog)s is called to import an mbox file for an '
               'entire group, with the -l flag.')
    p.add_argument('url', metavar='url',
                   help='The URL for the GroupServer site.')
    p.add_argument(
        '-l', '--list', dest='listId', default=None,
        help='The list to send the message to. By default it is extracted '
             'from the x-original-to header.')
    p.add_argument(
        '-f', '--file', dest='file', default='-', type=FileType('r'),
        help='The name of the mbox file. If omitted (or "%(default)s") '
             'standard-input will be read.')
    p.add_argument(
        '-c', '--config', dest='config', default=configFileName, type=str,
        help='The name of the GroupServer configuration file '
             '(default "%(default)s") that contains the token that will be '
             'used to authenticate the script when it tries to add each '
             'message to the site.')
    p.add_argument(
